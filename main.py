...
def approve_file(req: ApproveFileRequest, current_user: models.User = Depends(auth.get_current_admin_user)):
    ...
    if "auto_sorted" not in target_path.lower():
...
def download_apk():
    ...
    return FileResponse(apk_path, media_type='application/vnd.android.package-archive', headers={'Content-Disposition': 'attachment; filename=WebPortal.apk'})
