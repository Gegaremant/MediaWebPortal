...
function App() {
  ...
  const onTouchEndHandler = () => {
    ...
    if (isLeftSwipe && previewFile?.currentIndex < previewFile.items.length - 1) {
      setPreviewFile(prev => ({ ...prev, currentIndex: prev.currentIndex + 1 }))
    }
    if (isRightSwipe && previewFile?.currentIndex > 0) {
      setPreviewFile(prev => ({ ...prev, currentIndex: prev.currentIndex - 1 }))
    }
  }
      ...
      {activeView === 'photos' ? renderGalleryFullscreen(galleryPhotos, 'photos') : activeView === 'videos' ? renderGalleryFullscreen(galleryVideos, 'videos') : activeView === 'dashboard' ? (
        <div style={{ width: '100%', marginBottom: '2rem' }}>
          ...
          >
            ...
            <div key="sorter" className="file-card" style={{ flexDirection: 'column', alignItems: 'stretch', padding: '1rem', height: '100%' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: '#94a3b8', marginTop: '0.5rem', alignItems: 'center' }}>
                <span>Скорость: {latestMetrics?.sorting?.speed || 0} ф/с</span>
                <input 
                  type="range" 
                  min="0" max="5000" step="100" 
                  title="Задержка (мс)"
                  value={sorterState?.delay_ms || 0} 
                  onMouseUp={e => handleSorterControl(sorterState?.status, parseInt(e.target.value))}
                  onChange={e => setSorterState(prev => ({...prev, delay_ms: parseInt(e.target.value)}))}
                  style={{ width: '60px' }}
                />
                <span>{sorterState?.delay_ms || 0} мс</span>
                <span>Всего: {latestMetrics?.sorting?.total_moved || 0}</span>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', alignItems: 'stretch' }}>
          ...
          <div className="file-card" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '1rem' }}>
            <h3>📱 Приложение Android</h3>
            <p style={{ color: 'var(--text-secondary)' }}>Скачайте установочный APK-файл для мобильных устройств на Android.</p>
            <a href="/api/download/apk" download style={{ display: 'inline-block', textDecoration: 'none' }}>
              <button className="path-btn" style={{ background: '#10b981', color: 'white', padding: '0.8rem 1.5rem', width: 'fit-content' }}>
                Скачать APK (WebPortal.apk)
              </button>
            </a>
          </div>
